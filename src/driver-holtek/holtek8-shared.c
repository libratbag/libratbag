...
holtek8_write_macro_data(struct ratbag_device *device, struct holtek8_macro_event *macro_events)
{
	...
	while (events_to_write < HOLTEK8_MAX_MACRO_EVENTS && *((uint16_t*) &macro_events[events_to_write]) != 0)
		events_to_write += 1;
...
holtek8_macro_to_events(const struct ratbag_button *button, struct holtek8_macro_event *raw)
{
	...
	for (i = 0; i < MAX_MACRO_EVENTS; i++) {
		...
		if (event_i+2 >= HOLTEK8_MAX_MACRO_EVENTS)
			return -EOVERFLOW;
